...
namespace day10_csharp
{
	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			int second = 1;
			while (!ProcessMovement()) { second++; }
			Draw("day10.png");
			System.Console.WriteLine(second);
		}

		static bool ProcessMovement()
		{
			int connected = 0;
			...
			for (int i = 0; i < points.Length; i++)
			{
				foreach (Star cmp in points)
				{
					if (points[i].Position.X == cmp.Position.X && points[i].Position.Y == cmp.Position.Y)
					{
						continue;
					}
					if (Math.Abs(points[i].Position.X - cmp.Position.X) <= 1 && Math.Abs(points[i].Position.Y - cmp.Position.Y) <= 1)
					{
						connected++;
						break;
					}
			...
			return connected >= points.Length - 1;
		}

		static void Draw(string filename)
		{
			...
			using (FileStream png = new FileStream(filename, FileMode.OpenOrCreate, FileAccess.Write))
			{
				using (var image = new Bitmap(maxX / reductionFactor + pixelSize, maxY / reductionFactor + pixelSize))
				{
					var graphics = Graphics.FromImage(image);
					graphics.FillRectangle(Brushes.Black, new RectangleF(0, 0, maxX, maxY));
					graphics.FillRectangles(Brushes.White, points.Select(s => new Rectangle((s.Position.X - minX) / reductionFactor, (s.Position.Y - minY) / reductionFactor, pixelSize, pixelSize)).ToArray());
					image.Save(png, ImageFormat.Png);
				}
			}
		}

		static string input = @"position=<-42346,  10806> velocity=< 4, -1>
