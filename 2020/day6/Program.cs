...
var sums = Regex.Split(Input.Value, @"\r?\n\r?\n").Select(
        group =>
        {
            ...
            foreach (char c in group)
            {
                if (c >= 'a' && c <= 'z')
                {
                    int count = set.ContainsKey(c) ? set[c] : 0;
                    count++;
                    set[c] = count;
                }
            }
            (int, int) result = (set.Count, set.Where(kv => kv.Value == group.Split('\n', StringSplitOptions.RemoveEmptyEntries).Length).Count());
            return result;
        }
    ).Aggregate((0, 0), (total, next) => (total.Item1 + next.Item1, total.Item2 + next.Item2));

Console.WriteLine($"Star 1: {sums.Item1}");
Console.WriteLine($"Star 2: {sums.Item2}");

// Hack to have the input at the bottom of the file
static class Input
{
    public static string Value = @"wzaopvscxknyjtiul
belforsgdhuvmwc
