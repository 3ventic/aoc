...
Dictionary<string, Bag> bagsDict = new Dictionary<string, Bag>();
var goldenBag = NewBag("shiny", "gold");

var bags = Input.Value.Split(new string[] { "\r\n", "\n" }, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).Select(b =>
{
    var parts = b.Split(new string[] { " bags contain ", ", " }, StringSplitOptions.RemoveEmptyEntries);
    var bagType = parts[0].Split(' ');
    var bag = NewBag(bagType[0], bagType[1]);
    for (int i = 1; i < parts.Length; i++)
    {
        if (parts[i].StartsWith("no "))
        {
            continue;
        }
        bagType = parts[i].Split(' ');
        var cBag = NewBag(bagType[1], bagType[2]);
        bag.Bags[cBag] = int.Parse(bagType[0]);
    }
    return bag;
}).ToArray();

var star1 = bags.Where(b => b.FlattenBags().ContainsKey(goldenBag)).Count();
var star2 = goldenBag.FlattenBags().Values.Sum();

Console.WriteLine($"Star 1: {star1}");
Console.WriteLine($"Star 2: {star2}");

Bag NewBag(string modifier, string color)
{
    var key = $"{modifier} {color}";
    var bag = bagsDict.TryGetValue(key, out Bag b) ? b : new Bag()
    {
        Modifier = modifier,
        Color = color,
    };
...
class Bag
{
    ...
    public Dictionary<Bag, int> Bags { get; } = new Dictionary<Bag, int>();

    public Dictionary<Bag, int> FlattenBags()
    {
        Dictionary<Bag, int> bags = new Dictionary<Bag, int>();
        foreach (var bag in Bags)
        {
            var count = bags.TryGetValue(bag.Key, out int v) ? v + bag.Value : bag.Value;
            bags[bag.Key] = count;

            var innerBags = bag.Key.FlattenBags();
            foreach (var innerBag in innerBags)
            {
                var innerBagValue = innerBag.Value * bag.Value;
                var innerCount = (bags.TryGetValue(innerBag.Key, out int iv) ? iv + innerBagValue : innerBagValue);
                bags[innerBag.Key] = innerCount;
            }
        }
        return bags;
    }
    ...
    {
        return $"{Modifier} {Color}".GetHashCode();
    }
    ...
    {
        return obj is Bag b && b.GetHashCode() == GetHashCode();
    }
}
...
static class Input
{
    public static string Value = @"plaid fuchsia bags contain 5 light violet bags, 1 light yellow bag.
